...
description: Use when editing prose/text drafts (blog posts, docs, emails, PR descriptions, marketing copy, essays, any long-form writing) to sound less AI-generated and more human, or when asked to detect/flag AI-slop patterns in a piece of writing without rewriting it. Distinct from the "no-ai-slop" skill, which de-genericizes app/UI builds (visual design, layout, code structure) — this one is for text only.
---
...
4. For an edit, make the minimum effective changes, then check the edited draft against `references/eval.md` yourself.
...
Ported from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop). See also the sibling `no-ai-slop` skill for app/UI de-genericization — that one scores a *built product*; this one edits *text*.
